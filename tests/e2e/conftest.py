...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        help="Claude model to use for tests",
    )
    parser.addoption(
        "--e2e-verbose",
        action="store_true",
        default=os.environ.get("E2E_VERBOSE", "").lower() == "true",
        help="Enable verbose E2E output",
    )
...
def claude_runner(project_root, e2e_timeout, e2e_model, e2e_verbose, e2e_enabled):
    """Create Claude Code runner for tests."""
    if not e2e_enabled:
...
@pytest.fixture(scope="session")
def installed_plugin(claude_runner, project_root, e2e_enabled):
    """Install the plugin once for all tests."""
    if not e2e_enabled:
        pytest.skip("E2E tests disabled")

    # Install plugin from project root
    result = claude_runner.install_plugin(".")

    if not result["success"]:
        pytest.fail(f"Failed to install plugin: {result['error']}")
