...
update_version_in_file() {
    ...
    case "$file" in
        ...
        *marketplace.json)
            if command -v jq &> /dev/null; then
                local tmp=$(mktemp)
                jq ".version = \"$new_version\" | .plugins[0].version = \"$new_version\"" "$file" > "$tmp" && mv "$tmp" "$file"
            else
    ...
    success "Updated $(basename "$file") to $new_version"
}
...
check_versions() {
    ...
    if [[ -z "$source_version" ]]; then
        ...
        error "Create VERSION file with: echo '1.0.0' > VERSION"
        return 1
    fi
    ...
    local files=("$PLUGIN_JSON" "$MARKETPLACE_JSON")
...
set_version() {
    ...
    # Create or update VERSION file (source of truth)
    echo "$new_version" > "$VERSION_FILE"
    success "Created/updated VERSION file"
